#!/usr/bin/env bash
# ISO 回退到 tanh（不用 ReLU），seed 1/2/3，四档强度。
...
for s in 0.0001 0.001 0.01 0.1; do
  for seed in $SEEDS; do
    echo "  -> iso tanh strength=$s seed=$seed"
    "$PY" main.py --env "$ENV" --algorithm base --repeat_idx 0 --seed "$seed" \
        --learning_rate 0.0003 --num_steps 10000000 \
        --iso_reg "$s" \
        --wandb --wandb_online \
        --save_suffix "iso_${s}_seed${seed}" \
        > "logs/reg_full/iso_${s}_seed${seed}.log" 2>&1 &
  done
done
...
wait
echo "=== ISO tanh（seed 1/2/3）四档强度跑完 ==="
